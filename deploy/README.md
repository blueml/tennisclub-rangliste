...
- Eine Domain, die auf die öffentliche IP des Hosts zeigt (z. B. `rangliste.comunicar.de`).
...
In `deploy/nginx/nginx.conf` alle Vorkommen von `rangliste.comunicar.de` durch
...
docker run --rm \
  ...
  -d rangliste.comunicar.de --agree-tos -m blueml@blueml-it.de --no-eff-email
...
Die Anwendung ist danach unter `https://rangliste.comunicar.de` erreichbar.
